...
public class HashMap implements Map {
	...
	private void resize(int newCapacity) throws OverflowException {
		...
		// Create a new table with length = newCapacity		
		Entry[] newTable = new Entry[newCapacity];
		
		// Re-hash all entries in the old table to the new one
		Entry tempNext, tempOrigin;
		int newBucketIndex;
		for (int i = 0; i < this.table.length; i++){
			for(Entry entryToMove = table[i]; entryToMove != null;) {
				tempNext = entryToMove.next;
				//find new index
				newBucketIndex = indexFor(entryToMove.key, newCapacity);
				//put to new table
				tempOrigin = newTable[newBucketIndex];
				entryToMove.next = tempOrigin;
				newTable[newBucketIndex] = entryToMove;
				//progress
				entryToMove = tempNext;
			}			
		}                  
		
		// Assign new table to table
		this.table = newTable;
